...
CleanRegistryCmd = 
ifeq ($(GOOS),windows)
	BuildTarget = .\bin\terraform-provider-$(name).exe
	LocalRegistry = $(APPDATA)\terraform.d\plugins\registry.terraform.io\${group}\$(name)\$(version)\$(GOOS)_$(GOARCH)
	...
	BuildTarget = ./bin/terraform-provider-$(name)
	LocalRegistry = ~/.terraform.d/plugins/registry.terraform.io/$(group)/$(name)/$(version)/$(GOOS)_$(GOARCH)/
